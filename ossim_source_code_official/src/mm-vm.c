...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
{
  ...
  if(inc_vma_limit_ret == 0) {//not increase limit
...
{
  struct vm_rg_struct *removedItem = malloc(sizeof(struct vm_rg_struct));
...
int pgalloc(struct pcb_t *proc, uint32_t size, uint32_t reg_index)
{
  ...
  printf ("Used frames: ");
  struct framephy_struct *usedlist = proc->mram->used_fp_list;
  while (usedlist) {
    printf("%d ", usedlist->fpn);
    usedlist = usedlist->fp_next;
  }
  printf("\n");
...
int pgfree_data(struct pcb_t *proc, uint32_t reg_index)
{
  ...
  printf("free list: ");
  while (freelist) {
    printf("%ld - %ld; ", freelist->rg_start, freelist->rg_end);
...
int pg_getpage(struct mm_struct *mm, int pgn, int *fpn, struct pcb_t *caller)
{
  ...
  { /* Page is not online, make it actively living */
    ...
    /* Update page table */
    pte_set_swap(&mm->pgd[vicpgn], PAGING_SWPTYPE(swpfpn), PAGING_SWP(swpfpn));
  ...
  while (rgit != NULL)
  {
    ...
    { /* Current region has enough space */
      ...
      if (rgit->rg_start + size < rgit->rg_end)
      {
        ...
        struct vm_rg_struct *newrgit = malloc(sizeof(struct vm_rg_struct));
